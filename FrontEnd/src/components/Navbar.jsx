...
const Navbar = () => {
 const { token, handleLogout,auth } = useContext(AuthContext)
    ...
    return (
        <header className='bg-white/50 backdrop-blur-md sticky top-0 z-50 shadow-md'>
            <nav className='px-12 py-4 flex justify-between items-center '>
                ...
                {
                    auth && <button onClick={handleLogout} className="bg-orange-500 hover:bg-orange-600 transition text-white py-1 px-3 rounded-xl font-semibold">Logout</button>
                }
                </div>
            </nav>
