...
const MyRecipes = () => {
  ...
  async function getRecipes() {
    try {
      ...
      // console.log(data);
      myRecipes = data.recipes.filter((recipe) => {
        return recipe.userId == userData.id
      })
...
if (recipes.length === 0) {
    ...
    <section className='min-h-screen flex justify-center items-center bg-[#F9F7F2]'>
      <div className='text-center space-y-4'>
        ...
        <p className='text-gray-500 text-lg mb-6'>
          Start by adding your first recipe 🍽️
        </p>
               <Link to="/add-recipe">
  ...
  return (
    <>
      <section className='bg-[] py-12 relative'>
        <div className='container'>
          <h2 className='text-2xl text-black font-semibold  hover:text-orange-500 w-fit'>My Recipes</h2>
                ...
                return <div key={recipe?._id} className='shadow-md bg-white rounded-2xl group overflow-hidden hover:scale-105 transition-transform duration-200 '>
                  ...
                  <div className='px-4 flex flex-col gap-2 pb-4'>
                    ...
                    <div className='flex flex-col gap-2'>
                      <Link to={`/edit-recipe/${recipe._id}`} className="flex justify-between text-md">Edit Your Recipe <FaEdit className='text-2xl text-gray-500 font-bold' /></Link>
                      <Link className="flex justify-between text-md" onClick={() => { setPopupDelete(true), setId(recipe._id) }}>Delete You Recipe<MdDelete className='text-2xl text-gray-500 font-bold' /></Link>
                    </div>
                  </div>
                </div>
              })
      ...
      {
        popupDelete ? <DeletePopup handleDelete={() => { deleteRecipeById(id) }} popupDelete={popupDelete} setPopupDelete={setPopupDelete}></DeletePopup> : ""
      }
    </>
  )
}
